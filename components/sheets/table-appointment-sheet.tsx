...
export const TableAppointmentSheet = (props: Props) => {
  ...
  const canJoinMeeting = appointment.appointment_type === "virtual" && appointment.meeting_link;
        ...
        <div className="custom-scrollbar flex h-full flex-col overflow-y-scroll rounded-lg border border-gray-200 bg-white p-6 shadow-lg">
          <SheetHeader className="mb-6 p-0">
            <div className="flex items-center justify-between">
              <SheetTitle className="text-xl font-semibold text-gray-900 flex items-center space-x-3">
              ...
              {appointment.appointment_type === "virtual" ? (
                <div className="bg-blue-50 rounded-lg p-4 space-y-3">
                  ...
                  <p className="text-sm text-blue-700">
                    {appointment.location || "Online consultation via video call"}
                  </p>
                ...
                <div className="bg-green-50 rounded-lg p-4 space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium text-green-900">Home Visit</span>
                  </div>
                  <p className="text-sm text-green-700">
                    {appointment.home_address || appointment.additional_address || "Home address to be confirmed"}
                  </p>
                </div>
              ) : (
                <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium text-gray-900">Hospital Visit</span>
                  </div>
                  <div className="space-y-1">
                    <p className="font-medium text-gray-900">
                      {appointment.hospital_info?.name || appointment.hospital_name || "Hospital name not specified"}
                    </p>
                    <p className="text-sm text-gray-600">
                      {appointment.hospital_info?.address || appointment.hospital_address || "Address not specified"}
                    </p>
                    ...
                    {appointment.hospital_info?.contact_person && (
                      <div className="flex items-center space-x-2 text-sm text-gray-600">
                        ...
                        <span>Contact: {appointment.hospital_info.contact_person}</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              {appointment.caregiver ? (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium capitalize">{appointment.caregiver.user_type}</p>
                    </div>
                    <div>
                ...
                <div className="bg-yellow-50 rounded-lg p-4 flex items-center space-x-3">
                  ...
                  <span className="text-yellow-800">No caregiver assigned yet</span>
                </div>
              )}
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">
                      {user?.dob ? formatDate(new Date(user.dob)) : "Not provided"}
                    </p>
                  </div>
                  <div>
              ...
              {appointment.contact && (
                <div className="bg-blue-50 rounded-lg p-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-blue-900">Contact for this appointment:</span>
                  </div>
                  <p className="text-sm text-blue-700 ml-6">{appointment.contact}</p>
                </div>
              )}
            </div>

            {/* Additional Notes */}
            {(appointment.additional_note && appointment.additional_note !== "No additional note") && (
              <div className="space-y-3">
                <h3 className="text-lg font-semibold text-gray-900 flex items-center space-x-2">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  <p className="text-sm text-gray-700">{appointment.additional_note}</p>
                </div>
              </div>
            )}
