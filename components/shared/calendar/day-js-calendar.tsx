...
export default function DayjsCalendar() {
  ...
  useEffect(() => {
    const enrichAndTransform = async () => {
      ...
      setIsEnriching(true);
      try {
        const { calendarEvents } = await transformAppointmentsWithUserDetails(
        ...
        <div className="relative bg-white pt-[10px]">
          ...
          {events
            ...
            .map((event) => {
              ...
              const startCombined = combineDateAndTime(
                event.start,
                event.startTime
              );
              ...
              const minutesFromStart = Math.max(
                0,
                (eventStart.hour() - dayStartHour) * 60 + eventStart.minute()
              );
              ...
              const durationMinutes = Math.max(
                1,
                eventEnd.diff(eventStart, "minute")
              );
